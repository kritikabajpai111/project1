...
//        exit(print_r($data,true));
		...
		if($data['success']){//if there is an error, return the error stuff
			//$return_result['DataObject']=(isset($data['object']))?ucwords(str_replace('_',' ',$data['object'])):'';
			$return_result['Data']=(isset($data['data']))?$data['data']:'';
						
		}
			$return_result['Success']=(isset($data['success']))?'true':'false';
			$return_result['Message']=(isset($data['success']))?$data['success']:(isset($data['message'])?$data['message']:'');
			$return_result['ErrorCode']=(isset($data['error']))?$data['error']:'0';
                       
		if(!$data['success']){//if there is an error, return the error stuff
			$return_result['ErrorCode']=(isset($data['error']))?$data['error']:'0';
			$return_result['ErrorType']=(isset($data['type']))?$data['type']:'';
			$return_result['DataObject']=(isset($data['object']))?str_replace(' ','_',$data['object']):'';
			$return_result['DataObjectAttribute']=(isset($data['attribute']))?$data['attribute']:'';
		}
		
		if(strpos($return_result['ErrorCode'], '|') !== false) {//pipe delimited errors defined in Application/language/english/rest_controller_langu
		
		$rest_error=explode('|',$return_result['ErrorCode']);
		$return_result['ErrorCode'] = $rest_error[0];
		$return_result['Message'] = $rest_error[1];
		$return_result['ErrorType']='connection';
		}
