...
abstract class REST_Controller extends CI_Controller {
    ...
    public function __construct($config = 'rest')
    {
        ...
        $this->_direct_detect_output_format();
        ...
        $this->_get_args = array_merge($this->_get_args, $this->uri->ruri_to_assoc(), $this->uri->ruri_to_assoc(2));
    ...
    /**
     * Detect which format should be used to output the data before anything else
     *
     * @access protected
     * @return mixed|NULL|string Output format
     */
    protected function _direct_detect_output_format()
    {
        // Concatenate formats to a regex pattern e.g. \.(csv|json|xml)
        $pattern = '/('.implode('|', array_keys($this->_supported_formats)).')($|\/)/';
        $matches = [];
//exit($pattern.' :: '.$this->uri->uri_string());
        // Check if a file extension is used e.g. http://example.com/api/index.json?param1=param2
        if (preg_match($pattern, $this->uri->uri_string(), $matches)) {
        $this->uri->uri_string = str_replace("/".$matches[1],"",$this->uri->uri_string()).'/format/'.$matches[1]; //_set_uri_string($uri);
//exit(print_r($matches,true).' :: '.$this->uri->uri_string());
//            return $matches[1];
        }
...
//exit($pattern.' :: '.$this->uri->uri_string());
    ...
    /**
     * imports resources into controller
     *
     * @access protected
     * @return void
     */
    public function import($base, $class){
        try {
            // check if file exists else throw exception
            if(!file_exists(APPPATH.'/'.$base.'/'.$class.'.php'))
                throw new Exception("class '".$class."' not found");
            // include resource
            include_once(APPPATH.'/'.$base.'/'.$class.'.php');
        } catch (Exception $e){
            exit('error: '.print_r($e->getMessage(),true)."\n");
        }
    }
    ...
    protected function get_input($key){
            return (key($this->_get_args)!=$key)?key($this->_get_args):$this->get($key);
    }
